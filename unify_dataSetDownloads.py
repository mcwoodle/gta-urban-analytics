...
def load_mapping():
    if not os.path.exists(mapping_file):
        logging.warning("No crime_mapping.json found.")
        return {}
...
for f in glob.glob(os.path.join(data_dir, 'Durham_*.csv')):
    ...
    out = pd.DataFrame({
        ...
        'mapped_crime_category': df.get('offence', pd.Series(dtype=str)).apply(map_crime),
...
if os.path.exists(halton):
    ...
    out = pd.DataFrame({
        ...
        'mapped_crime_category': df.get('DESCRIPTION', pd.Series(dtype=str)).apply(map_crime),
        'occurrence_date': dates,
        'lat': df.get('Latitude'),
...
for f in glob.glob(os.path.join(data_dir, 'York_*.csv')):
    ...
    out = pd.DataFrame({
        ...
        'mapped_crime_category': crime_type.apply(map_crime),
        'occurrence_date': dates,
        'lat': lats_series,
        'lon': lons_series,
