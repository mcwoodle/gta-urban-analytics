...
from gta_urban_analytics.transform.crime.crime_groups import assign_crime_group
...
TRANSFORM_STEPS = [
    ...
    # Must run AFTER dedup: dedup overwrites mapped_crime_category to MULTIPLE
    # for multi-offence incidents, so the bucket must be derived from the final
    # category. crime_group then persists into unified_data.csv downstream.
    ("Assigning crime groups",          lambda df: assign_crime_group(df)),
]
